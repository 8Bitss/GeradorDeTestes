...
Para cada **disciplina**, cadastra-se: uma lista de questões objetivas, e matérias pertencentes. 
...
#### Requisito 2.1: Cadastrar Matérias.
	Critérios:
- O registro de **matéria** é composto por:
	- Nome
	- **Disciplina**
	- Série
...
#### Requisito 3.1: Cadastrar Questões.
	Critérios:
- O registro de questão é composto por:
	- **Matéria**
	- Enunciado
	- Alternativas
...
#### Requisito 4.1: Gerar Testes.
	Critérios:
- O registro de teste é composto por:
	- Título
	- **Disciplina**,
	- **Matéria** e Série 
...
Critérios:	
- [x] A visualização dos registros deve ser apresentada utilizando o componente DataGridView
- [x] As telas de cadastro devem aparecer centralizadas
- [x] Não deve permitir redimensionar telas de cadastro
- [x] Não deve permitir minimizar ou maximizar telas de cadastro
- [x] As telas de cadastro devem ser dialogs
- [x] As telas de cadastro devem ter um título
- [x] Os elementos das telas de cadastro devem seguir um padrão de posicionamento
- [x] Os elementos das telas de cadastro devem estar alinhados
- [x] O nome do sistema deve ser apresentado na tela principal
- [x] As notificações para usuário devem ser apresentadas seguindo um padrão
- [x] As telas de cadastro devem ter uma opção que fecha a janela e não grava as alterações
- [x] As telas de cadastro devem ter uma opção que grava as alterações
- [x] A tabulação dos campos deve seguir uma sequência lógica iniciando pelos campos superiores
- [x] As alterações realizadas devem ser gravadas e devem manter uma referência única em todas as telas
